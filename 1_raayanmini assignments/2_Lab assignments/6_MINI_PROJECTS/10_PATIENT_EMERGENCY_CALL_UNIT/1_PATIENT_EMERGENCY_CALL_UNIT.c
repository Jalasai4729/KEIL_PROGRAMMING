...
unsigned char str1[]="PATIENT CALL";
unsigned char str2[]="EMRGENCY UNIT";
unsigned char str3[]="DANGER !!";
unsigned char str4[]="NEED HELP";
unsigned char str5[]="NURSE IS ARRIVED";
unsigned char str6[]="START TREATMENT";
unsigned char str7[]="OUT OF DANGER";
unsigned char str8[]="EVERYTHING IS OK";
...
//OPERATION PHASE
	while (1)
	{
		...
		else if(flag==2)//START TREATMENT
		{
          ...
          km_write_string(str5);//DORECTION
   			  km_write_cmd(0xc0);//CURSOR ON 1st ROW STARTING POSITION
          km_write_string(str6);//DORECTION
		...
		else if(flag==3)//EVERYTHING IS FINE
		{
			   km_write_cmd(0x01);//CLEAR LCD DISPLAY		
			    km_write_cmd(0x80);//CURSOR ON 1st ROW STARTING POSITION
          km_write_string(str7);//DORECTION
   			  km_write_cmd(0xc0);//CURSOR ON 1st ROW STARTING POSITION
          km_write_string(str8);//DORECTION
			
			      GPIOB_ODR |=(0x1<<14);//GREEN LED OFF negative logic
            GPIOB_ODR |=(0x1<<13);//RED LED OFF negative logic
				    GPIOB_ODR &=~(0x1<<12);//BUZZER LED OFF positive logic
             flag=0;
		     	x=0;
		}
	}
