...
/******** Initialization Phase **************************************************/
	  RCC_AHB1ENR |= (1 << 2);   // clock ENABLE PORT C
				RCC_APB2ENR |= (0X1 << 8);   // Clock to ADC1

	/*********** CONFIGARATION PHASE ************************************************/
	  GPIOC_MODER &= ~(3 << 2); // clear
		GPIOC_MODER |= (3 << 2); // set in Analog mode 11 PC1
	
		ADC1_SQR3 |= ( 0XB << 0);   // Loading the Channel Number
		ADC1_CR2 |= ( 0X1 << 0);    // ADON 
...
/********** OPERATION PHASE ********************************************************/	
	while(1)
	{
		...
		ADC1_CR2 |= (0x1 << 30); // Set the SWSTART 
		
		while(ADC1_SR & (0X1 << 1));//EOC 
