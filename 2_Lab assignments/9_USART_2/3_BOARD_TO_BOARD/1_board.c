...
#define EXTI_FTSR (*(int*)0x40013C0C)//FALLING TRIGGER SELECTION REGISTER
#define EXTI_IMR (*(int*)0x40013C00)//INTERRUPT MASK REGISTER
...
int main()
{
	...
	LED_INIT();       //CLOCK FOR PRORT B
	SWITCH_INIT();    //CLOCK FOR PORT C
  stk_init();
...
void LED_INIT()
{
		RCC_AHB1ENR |= (1 << 1);//SET CLOCK PORRT B      
